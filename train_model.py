...
def train_model(
    ...
    optimizer = torch.optim.AdamW(
        model.parameters(), lr=learning_rate, weight_decay=l2_regularisation
    )
    ...
    for epoch in range(num_epochs):
        ...
        for (imageB, imageA), mask in tqdm(
        ...
        with torch.no_grad():  # Disable gradient computation for validation to save memory and speed up
            for (images_b, images_a), masks in tqdm(
                val_loader, desc=f"Epoch {epoch + 1}/{num_epochs} Validation"
            ):
                # Move data to device
                images_b, images_a, masks = (
                    images_b.to(device),
                    images_a.to(device),
                    masks.to(device),
                )
